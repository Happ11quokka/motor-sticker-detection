...
def create_gradio_interface():
    ...
    with gr.Blocks(title="Motor Sticker Detection Dashboard") as demo:
        ...
        # 데모 시연용 사전 대화 (불량품 비율 Q&A)
        demo_chat_history = [
            {"role": "user", "content": "불량품 비율은?"},
            {"role": "assistant", "content": (
                "현재까지 분석된 결과를 정리해드리겠습니다. 😊\n\n"
                "📊 **전체 통계**\n"
                "- 총 분석 이미지: 52건\n"
                "- 스티커 발견: 48건\n"
                "- 미확인(스티커 없음/인식 실패): 4건\n\n"
                "🏷️ **스티커 분석 결과 (스티커 있는 48건 기준)**\n"
                "- 정상(초록색): 39건 (81.3%)\n"
                "- 경미한 불량(노란색): 6건 (12.5%)\n"
                "- 심각한 불량(빨간색): 3건 (6.3%)\n\n"
                "⚠️ **불량률**\n"
                "- 전체 불량품: 9건\n"
                "- **불량률: 18.8%** (경미 12.5% + 심각 6.3%)\n\n"
                "심각한 불량 3건은 우선 재검토를 권장드리며, 미확인 4건은 "
                "\"미확인 항목 재분석해줘\" 명령으로 다시 분석해보실 수 있습니다."
            )},
        ]
